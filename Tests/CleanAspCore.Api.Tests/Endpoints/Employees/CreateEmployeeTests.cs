...
public sealed class CreateEmployeeTests(TestWebApiFixture testWebApiFixture) : ApiTestBase(testWebApiFixture)
{
    ...
    public async Task CreateEmployee_InvalidEmail_ReturnsBadRequest()
    {
        ...
        var request = new { firstName = "Mary", lastName = "Poppins", email = "this is not a valid email address", gender = "Female", departmentId, jobId };

        //Act
        var response = await Sut.CreateUntypedClientFor(ClaimConstants.WriteRole).PostAsJsonAsync("/employees", request);

        //Assert
        await response.AssertBadRequest("Email");
    }
